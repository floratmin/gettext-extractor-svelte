...
function getExtractor(svelteFile: string, options: ICustomJsExtractorOptions): SvelteGettextExtractor {
  ...
  extractor.createSvelteParser().addExtractor(callExpressionExtractor('_', options)).parseSvelteString(svelteFile, 'src/App.svelte');
  return extractor;
}
...
describe('Extract translation functions to gettext and to function dict', () => {
  test('Comments are strings', () => {
    ...
    expect(getMessagesFromExtractor(extractor)).toEqual(
      sortMessages([
        {
          text: 'Foo',
          context: 'Context',
          comments: ['Comment'],
          references: ['src/App.svelte:3'],
          textPlural: null,
        },
        {
          text: 'Bar',
  ...
  test('Comments are objects with fallback', () => {
    ...
    expect(getMessagesFromExtractor(getExtractor(svelteFile, options))).toEqual(
      sortMessages([
        {
          text: 'Foo',
          context: 'Context',
        ...
        },
        {
          text: 'Bar',
          context: 'Context',
          comments: ['Comment', '{BAR}: Bar comment'],
          references: ['src/App.svelte:8'],
          textPlural: null,
        },
        {
        ...
        },
        {
          text: 'Bax',
          context: 'Context',
          comments: ['Comment', '{BAX}: Bax comment'],
          references: ['src/App.svelte:14'],
          textPlural: null,
        },
        {
  ...
  test('Read files with glob', () => {
    ...
    const extractor = new SvelteGettextExtractor();
    extractor.createSvelteParser().addExtractor(callExpressionExtractor('t', options)).parseFilesGlob('./**/[!Error]*.svelte');
      ...
      sortMessages([
        {
          ...
          textPlural: null,
          context: 'app',
          references: ['tests/App.svelte:4'],
          comments: ['Welcoming the user', '{NAME}: Name of user'],
        },
        {
          text: 'Hello World',
          textPlural: null,
          context: 'app',
          references: ['tests/App.svelte:8'],
        ...
        },
        {
          ...
          textPlural: null,
          context: 'Context',
          comments: ['Comment', 'path: https://www.example.com'],
          references: ['tests/Readme.svelte:5'],
        },
        {
          text: 'Foo',
          textPlural: null,
          context: null,
          references: ['tests/App.svelte:9', 'tests/Readme.svelte:14'],
          comments: [],
        },
        {
          text: 'Bar',
          ...
          context: null,
          comments: [],
          references: ['tests/Readme.svelte:15'],
        },
        {
          text: 'Baz',
          textPlural: null,
        ...
        },
        {
          ...
          context: 'Context',
          comments: ['Comment'],
          references: ['tests/Readme.svelte:17'],
        },
        {
          text: 'Hello {PLACE}',
          textPlural: null,
          context: 'Context',
          comments: ['Multiline', 'Comment', 'messageformat: This could be a messageformat function', '{PLACE}: The place where you are'],
          references: ['tests/Readme.svelte:18'],
        },
      ]),
    );
    expect(extractor.getFunctions()).toEqual({
  ...
  test('Throw error on file Error.svelte', () => {
    ...
    expect(() =>
      new SvelteGettextExtractor().createSvelteParser().addExtractor(callExpressionExtractor('_', options)).parseFile(`tests/Error.svelte`),
    ).toThrow();
  });
  test('Extracts from plain js', () => {
    ...
    const jsString = i`
        ...
        `;
    const extractor = new SvelteGettextExtractor();
    extractor.createJsParser().addExtractor(callExpressionExtractor('_', options)).parseString(jsString, './src/file.js');
  ...
  test('Extracts function expression, multiple positions, restrict parsing to src/file.ts', () => {
    ...
    const extractor = new SvelteGettextExtractor();
    extractor.createJsParser().addExtractor(callExpressionExtractor('_', options)).parseString(jsString, './src/file.ts');
  ...
  test('Extracts class declaration and class expression', () => {
    ...
    const findTranslationClassExpression = functionExtractor.variableDeclaration(
      'Foo2',
      functionExtractor.classExpression(undefined, [functionExtractor.methodDeclaration('bar2', true)]),
    );
    const options: ICustomJsExtractorOptions = {
      arguments: {
        text: 0,
...
let Foo2 = class Bar2 {
    ...
    expect(extractor.getFunctions()).toEqual({
      'src/file.ts': [
        {
          functionString: `bar(a: string): string {
        return a;
    }`,
          identifier: 'functionIdentifier',
          definition: true,
        ...
        {
          functionString: `function(b: string): string {
        return b;
    }`,
          identifier: 'functionIdentifier',
          definition: true,
        ...
        {
          functionString: `bar2(d: string): string {
        return d;
    }`,
          identifier: 'functionIdentifier2',
          definition: true,
  ...
  test('Extracts class expression, readme example', () => {
    ...
    const findTranslationClassExpression = functionExtractor.variableDeclaration(
      'foo',
      functionExtractor.classExpression('Bar', [functionExtractor.methodDeclaration('bar', true)]),
    );
    const options: ICustomJsExtractorOptions = {
      arguments: {
        text: 0,
    ...
    const jsString = `let foo = class Bar {
    ...
    expect(extractor.getFunctions()).toEqual({
      'src/file.ts': [
        {
          functionString: `bar(translationObject: any) {
        return 'I am translated';
    }`,
          identifier: 'translatorFunction',
          definition: true,
...
export class Foo {
    ...
    expect(extractor.getFunctions()).toEqual({
      'src/file.ts': [
        ...
        {
          functionString: `set bar (b: string) {
        this.b = b;
    }`,
          identifier: 'functionIdentifier',
          definition: true,
  ...
  test('Extracts from object literal expression', () => {
    const functionExtractor = new FunctionExtractorBuilder();
    const findTranslationObject = functionExtractor.variableDeclaration(
      'foo',
      functionExtractor.objectLiteralExpression(
        [
          ...
          functionExtractor.propertyAssignment(
            'bax',
            functionExtractor.objectLiteralExpression([functionExtractor.propertyAssignment('bay', functionExtractor.arrowFunction(), true)]),
            true,
          ),
        ],
        true,
    ...
    expect(extractor.getFunctions()).toEqual({
      'src/file.ts': [
        ...
        {
          functionString: `bay: (c: string): string {
            return string => c
        }`,
          identifier: 'functionIdentifier',
          definition: true,
        ...
        {
          functionString: `bax: { // rendered comments
        /* rendered comments */
        bay: (c: string): string {
            return string => c
        }
  ...
  test('Extracts function expression from svelte files (looks like function declaration when between script tags)', () => {
    ...
    const extractor = new SvelteGettextExtractor();
    extractor.createSvelteParser().addExtractor(callExpressionExtractor('_', options)).parseString(svelteString, './src/App.svelte');
  ...
  test('Extracts imports', () => {
    ...
    const findFunctionDeclaration = functionExtractor.importDeclaration(
      'foo-module',
      functionExtractor.importClause('Foo', [functionExtractor.importSpecifier('bar', true)], true),
      true,
    );
    const options: ICustomJsExtractorOptions = {
      arguments: {
  ...
  test('Extracts imports supplied trough parser', () => {
    ...
    const findBarFunctionDeclaration = functionExtractor.importDeclaration(
      'bar-module',
      functionExtractor.importClause(undefined, [functionExtractor.importSpecifier('bar')]),
      true,
    );
    const findFooFunctionDeclaration = functionExtractor.importDeclaration('foo-module', functionExtractor.importClause('Foo', undefined), true);
    const options: ICustomJsExtractorOptions = {
      arguments: {
        text: 0,
        context: 1,
    ...
    const extractor = new SvelteGettextExtractor();
    extractor.createJsParser().addExtractor(callExpressionExtractor('_', options)).parseString(jsString, './src/file.js', { translatorFunctions });
  ...
  test('Extracts imports single', () => {
    ...
    const findFunctionDeclaration = functionExtractor.importDeclaration(
      './foo-module',
      functionExtractor.importClause(undefined, [functionExtractor.importSpecifier('bar', true)], true),
      true,
    );
    const options: ICustomJsExtractorOptions = {
      arguments: {
  ...
  test('Retrieves messages and functions dict from last call', () => {
    ...
    expect(extractor.getLastAddedMessages()).toEqual([
      {
        text: 'Foo2',
        context: 'Context',
        comments: ['Comment'],
        references: ['src/App2.svelte:5'],
  ...
  test('Extracts new expressions', () => {
    ...
    const findFunctionDeclaration = functionExtractor.variableStatement(
      [functionExtractor.variableDeclaration('foo', functionExtractor.newExpression('Foo', true), true)],
      true,
    );
    const options: ICustomJsExtractorOptions = {
      arguments: {
  ...
  test('Extracts from js files', () => {
    ...
    const extractor = new SvelteGettextExtractor();
    extractor.createJsParser().addExtractor(callExpressionExtractor('_', options)).parseFilesGlob('./tests/**/*.js');
    expect(sortMessages(extractor.getMessages())).toEqual([
      {
        text: 'Foo',
        textPlural: null,
